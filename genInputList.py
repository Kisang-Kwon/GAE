...
def seperate_data(outdir, prefix, ligand_dict, ff_dir, mode):
    ...
    ff_list = [f.replace('_UNL.ff', '') for f in os.listdir(ff_dir) if f.endswith('.ff')]
    ...
    for pdb in ligand_dict.keys():
        if pdb in ff_list:
            ...
            print(f'[Notice] {pdb}_UNL.ff does not exist.')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--ffdir', dest='ffdir', required=True)
    ...
    seperate_data(args.outdir, args.prefix, ligand_dict, args.ffdir, args.mode)
